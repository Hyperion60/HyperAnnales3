...
EXTENSION_FILE = [
    ...
    ('Document Word', 'docx'),
    ('Objet 3D FBX', 'fbx'),
    ('Fichier audio FLAC', 'flac'),
    ('Image animée GIF', 'gif'),
    ('Ficher GIT', 'gitignore'),
    ('Clé GPG', 'gpg'),
    ('Fichier Graphe Junior', 'gra'),
    ('Archive Gzip', 'gz'),
    ('Fichier entête C', 'h'),
    ('Page HTML', 'html'),
    ('Calendrier ICS', 'ics'),
...
def category_select(request, context):
    ...
    if request.user.is_superuser and not int(request.POST['subject']):
        try:
            CreateSubject(context, request.POST['new_subject'], semester_obj.pk, year_obj.pk,
                                    School.objects.get(school__exact=request.user.school).pk)
        except School.DoesNotExist:
            context['errors'].append("Utilisateur invalide")
